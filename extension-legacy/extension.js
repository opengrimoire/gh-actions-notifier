...
class GitHubActionsIndicator extends PanelMenu.Button {
  ...
  _updatePanel(snapshot) {
    ...
    const activeCount = snapshot.history.filter(Core.isActiveRun).length;
    const failureCount = snapshot.history.filter(run => Core.isNegativeConclusion(run.conclusion)).length;
  ...
  _addRunItem(run, snapshot) {
    const severity = Core.runSeverity(run);
    const duration = Core.formatDuration(Core.durationSeconds(run));
    ...
    if (Core.isActiveRun(run)) {
      const watched = snapshot.watchedRunKeys.has(Core.runKey(run));
      const watchItem = new PopupMenu.PopupMenuItem(watched ? _('Unwatch success notification') : _('Watch success notification'));
      watchItem.connect('activate', () => this._extension.toggleWatch(run));
    ...
    if (Core.isNegativeConclusion(run.conclusion)) {
      const logExcerpt = snapshot.logExcerpts.get(Core.runKey(run));
...
class GitHubActionsNotifierExtension {
  constructor() {
    this._settings = null;
  ...
  toggleWatch(run) {
    ...
    this._state.watchedRunKeys = Array.from(watchedKeys);
    State.saveState(this._state);
    this._updateIndicator();
  }
  ...
  async loadFailedLog(run) {
    ...
    try {
      const client = new GitHub.GitHubClient(this._token, this._cancellable);
      const jobs = await client.listJobs(run.repoFullName, run.id);
      const failedJobs = jobs.filter(job => Core.isNegativeConclusion(job.conclusion));
      ...
      for (const job of failedJobs.slice(0, 3)) {
        ...
        const excerpt = Core.extractRelevantLog(logText, job.failedStepNames);
        if (excerpt) {
      ...
      this._logExcerpts.set(Core.runKey(run), joinedExcerpt || _('No relevant failed log lines were found.'));
      this._error = '';
      this._updateIndicator();
    } catch (error) {
  ...
  _processRuns(fetchedRuns) {
    ...
    for (const run of this._state.history) {
      previousRuns.set(Core.runKey(run), run);
    }
    ...
    for (const run of fetchedRuns) {
      const decision = Core.shouldNotifyRun({
        run,
        previousRun: previousRuns.get(Core.runKey(run)) || null,
        watchedRunKeys,
        notifiedKeys,
      ...
      if (decision.notify) {
        ...
        notifiedKeys.add(Core.notificationKey(run));
      }
    }

    const historyDays = this._settings.get_uint('history-days');
    this._state.history = Core.pruneRunHistory(
      Core.mergeRunHistory(this._state.history, fetchedRuns),
      historyDays
    );

    const liveRunKeys = new Set(this._state.history.map(Core.runKey));
    this._state.watchedRunKeys = Array.from(watchedRunKeys).filter(key => liveRunKeys.has(key));
    this._state.notifiedKeys = Array.from(notifiedKeys).slice(-1000);
  ...
  _handlePollError(error) {
    ...
    if (error instanceof GitHub.GitHubApiError && error.rateLimitReset !== null) {
  ...
  _getNormalPollInterval() {
    const hasActiveRuns = this._state.history.some(run => {
      return this._selectedRepos.includes(run.repoFullName) && Core.isActiveRun(run);
    });
  ...
  _updateIndicator() {
    ...
    this._indicator.update({
      ...
      history: (this._state && this._state.history ? this._state.history : []).filter(run => this._selectedRepos.includes(run.repoFullName)),
      watchedRunKeys: new Set(this._state && this._state.watchedRunKeys ? this._state.watchedRunKeys : []),
...
function init() {
  return new GitHubActionsNotifierExtension();
}
